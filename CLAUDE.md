...
- **Frete = motor de tabelas próprias** (`@emach/db/queries/shipping-quote`, puro) via adapter `lib/shipping/quote.ts`; substituiu o SuperFrete (#160/#242 — `lib/superfrete/`, `lib/origin-branch.ts`, env `SUPERFRETE_*`/`DEFAULT_BRANCH_ID` removidos). Cota por **zona de CEP de destino** × faixa de peso × caixa (`packItems` consolida múltiplos itens numa caixa real); **não usa origem/filial**. Sem cobertura (sem zona/faixa) ou item sem caixa (`out_of_catalog`) → `negotiate: true` = "Frete a combinar" — decisão consciente: `tool.overweightShippingAmount` **não** é mais consumido (item pesado sem caixa vira "a combinar"). `place-order > assertShippingQuoted` re-cota e valida por preço; motor local determinístico → o fail-open de API do SuperFrete deixou de existir. **Gotcha P0:** as tabelas de shipping (`carrier`/`carrierZone`/`carrierRate`/`shippingBox` + relations) precisam estar registradas no objeto `schema` do singleton `packages/db/src/index.ts` (fora do escopo do sync, igual ao gotcha do `packages/db/CLAUDE.md`) — senão `db.query.carrier` é `undefined` em runtime e a cotação quebra (`check-types` **não** pega).
